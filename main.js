...
btn5.addEventListener("click", () => {
    ...
    // Remove existing highest/lowest row if it exists
    const existingRow = document.getElementById("highestLowestRow");
    if (existingRow) {
        tbl.removeChild(existingRow);
    }

    // Create a new row for highest and lowest values
    const trHighestLowest = document.createElement("tr");
    trHighestLowest.id = "highestLowestRow";
    const tdLabel = document.createElement("td");
    const tdValues = document.createElement("td");

    tdLabel.style.fontWeight = "bold";
    tdLabel.innerHTML = "HIGHEST<br>LOWEST";
    tdValues.innerHTML = `${highest} <br> ${lowest}`;
    tdValues.style.textDecoration = "underline";

    trHighestLowest.appendChild(tdLabel);
    trHighestLowest.appendChild(tdValues);

    // Insert the row before the total row
